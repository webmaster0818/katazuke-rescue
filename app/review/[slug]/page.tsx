...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!r) return {};
  const url = `https://katazuke-rescue.com/review/${r.slug}/`;
  return {
    title: r.title,
    ...
    "@graph": [
      {
        ...
        headline: r.title,
        description: r.metaDescription,
        image: `https://katazuke-rescue.com${r.heroImage}`,
        dateModified: "2026-06-16",
        author: { "@type": "Organization", name: "片付けレスキュー" },
        publisher: { "@type": "Organization", name: "片付けレスキュー", url: "https://katazuke-rescue.com/" },
        mainEntityOfPage: `https://katazuke-rescue.com/review/${r.slug}/`,
      },
      {
        "@type": "FAQPage",
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-8">
      ...
      <section id="reviews" className="scroll-mt-20 mb-12">
        ...
        <figure className="mt-8">
          <img src={r.subImage} alt={r.subAlt} width={1200} height={630}
            className="w-full rounded-2xl border border-border object-cover aspect-[1200/630]" />
          <figcaption className="mt-2 text-xs text-text-muted text-center">不用品回収・遺品整理・片付けは作業量や間取り、品目によって費用が異なります。</figcaption>
        </figure>
      </section>
      ...
      <div className="mt-8 flex flex-wrap gap-4 text-sm">
        ...
        <Link href="/ranking/" className="text-primary hover:underline">片付け・不用品回収業者ランキングを見る</Link>
      </div>
    </article>
  );
}
